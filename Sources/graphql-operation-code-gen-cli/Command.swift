...
struct Command: AsyncParsableCommand {
    ...
    @Option(
        help:
            "Path to GraphQL operation file. Multiple files can be used - each file will be treated as a single operation.",
        completion: .file(),
        transform: URL.init(fileURLWithPath:)
    )
    var operationPath: [URL]
    ...
    func run() async throws {
        ...
        let operations = try operationPath.map { path in
            guard let file = try? FileHandle(forReadingFrom: path).readToEnd() else {
                throw ValidationError("Could not read file at \(path)")
            }

            guard let value = String(data: file, encoding: .utf8) else {
                throw ValidationError("Operation file could not be decoded as utf8")
            }
        ...
        let generator: OperationsGenerator
        do {
